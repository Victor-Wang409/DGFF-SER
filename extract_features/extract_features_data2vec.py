...
def batch_extract_and_save(input_dir, output_dir):
    ...
    print("正在加载 Data2Vec-large 模型...")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_name = "facebook/data2vec-audio-large-960h"

    processor = Wav2Vec2Processor.from_pretrained(model_name)
    model = Data2VecAudioModel.from_pretrained(model_name).to(device)
    model.eval()
    ...
    for audio_file in tqdm(audio_files, desc="特征提取进度"):
        ...
        try:
            ...
            # 转换为 numpy (float16) 并保存
            features_np = fused_features.cpu().numpy().astype(np.float16)
...
if __name__ == "__main__":
    ...
    OUTPUT_DIRECTORY = "/home/victor/Github/DGFF-SER/Features/data2vec_large_features"
